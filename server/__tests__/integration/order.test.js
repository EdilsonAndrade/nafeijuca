...
describe('ORDERS', () => {
  ...
  it('should return the total of the order', async () => {
    await truncate();
    const userMaster = await factory.create('User', {
      active: true,
      confirmed: true,
    ...
    const orderResponse = await request(app)
      .post(`/orders`)
      .send({
        productIds: [product1.id, product1.id, product2.id, product2.id],
        clientId: user.id,
        storeId: fakeStore.id,
        costTax: 10,
      })
    ...
    expect(clientOrders.body.length).toBe(1);
    expect(clientOrders.body[0].OrderProducts.length).toBeGreaterThan(0);
    expect(orderResponse.body).toHaveProperty('total');
    expect(clientOrders.body[0].total).toEqual(130);
    expect(clientOrders.body[0].subTotal).toEqual(120);
